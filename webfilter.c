...
// ** Windivert 홈페이지에 있는 Webfilter.c 를 바탕으로 수정 **

// 입력 데이터에서 앞에 http:// 가 붙는 경우 삭제 처리
// log.txt로 로그파일이 남도록 수정

// (1) 탐지
// TCP에서 URL을 추출, mal_site.txt에 속해있는지 이진검색을 통해 비교

// (2) 차단
// 기존 페이지 대신 block_data 코드를 전송해 차단돼었음을 가시화 

// (3) 개선
// 기존의 이진검색 알고리즘을 그대로 유지. / KMP 알고리즘 및 아호코라식 알고리즘 미구현 
...
int __cdecl main(int argc, char **argv)
{
    ...
    if (argc <= 1)
    {
        fprintf(log, "usage: %s blacklist.txt [blacklist2.txt ...]\n",
            argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    if (blockpage == NULL)
    {
        fprintf(log, "error: memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    {
        fprintf(log, "error: failed to open the WinDivert device (%d)\n",
            GetLastError());
        exit(EXIT_FAILURE);
    }
    fprintf(log,"OPENED WinDivert\n");
    ...
    while (TRUE)
    {
        ...
        {
            fprintf(log, "warning: failed to read packet (%d)\n",
                GetLastError());
            continue;
        }
            ...
            !BlackListPayloadMatch(blacklist, payload, (UINT16)payload_len))
        {
            ...
            {
                fprintf(log, "warning: failed to reinject packet (%d)\n",
                    GetLastError());
            }
            continue;
        }
        ...
        {
            fprintf(log, "warning: failed to send reset packet (%d)\n",
                GetLastError());
        }
        ...
        if (!WinDivertSend(handle, (PVOID)blockpage, blockpage_len, &addr,
                NULL))
        {
            fprintf(log, "warning: failed to send block page packet (%d)\n",
                GetLastError());
        }
        ...
        {
            fprintf(log, "warning: failed to send finish packet (%d)\n",
                GetLastError());
        }
    }
}
...
memory_error:
    fprintf(log, "error: failed to allocate memory\n");
    exit(EXIT_FAILURE);
}
...
static void BlackListRead(PBLACKLIST blacklist, const char *filename)
{
    ...
    if (file == NULL)
    {
        fprintf(log, "error: could not open blacklist file %s\n",
            filename);
        exit(EXIT_FAILURE);
    }

	printf("READ BLACKLIST\n");
    ...
    while (TRUE)
    {
        ...
        i = 0;
        domain[i++] = (char)c;
		while ((isalnum(c = getc(file)) || c == '-' || c == '.' || c == ':' || c=='/') && i < MAXURL)
        {
            domain[i++] = (char)c;
        }
        domain[i] = '\0';

		if (!strncmp("http://", domain, 7))  // http 처리 
		{
			strcpy(httpdomain, domain + 7);
			strcpy(domain, httpdomain);
		}
...
static BOOL BlackListPayloadMatch(PBLACKLIST blacklist, char *data, UINT16 len)
{
    ...
    fprintf(log,"URL %s/%s: ", domain, uri);
